...
from listings.models import Project, Category
...
class SmokeTestSuite(TestCase):
    ...
    signup -> login -> listing creation -> admin approval -> purchase -> webhook -> download -> payout request -> admin completion.
    """
    ...
    def test_end_to_end_flow(self):
        ...
        seller = User.objects.create_user(username="smoke_seller", email="seller@test.com", is_seller=True)
        ...
        res = self.api_client.post(
            reverse("project_submit", args=[project_id]),
        ...
        with patch("payments.mirpay.MirPayClient.create_payment") as mock_create_pay, \
            ...
            res = self.api_client.post(reverse("order_create"), data={"project_id": project.id}, format="json")
            self.assertEqual(res.status_code, 201)
            order_data = res.json()
        ...
        with patch("payments.views.mirpay.check_status") as mock_check_status, \
             patch("notifications.tasks.notify_user_task.delay") as mock_notify:
            mock_check_status.return_value = {"status": "success", "summa": "50000.00", "payid": "MP123"}
        ...
        with patch("notifications.tasks.notify_user_task.delay") as mock_notify:
            payout.status = PayoutRequest.Status.COMPLETED
            payout.save(update_fields=["status"])
            
            # Since admin completion is via Django Admin Action in reality,
            # we just test that the model enforces status changes properly or the logic in admin would do so.
